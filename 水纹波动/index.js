class WaterRipple {
    ...
    init(props) {
        ...
        // this.bridge.src = './images/bridge.png'
    ...
    render() {
        ...
        for (let i = 0; i < len; i += 4) {
            ...
            let left = Math.max(x - 1, 0),
                right = Math.min(x + 1, w - 1),
                top = Math.min(y + 1, h - 1),
                bottom = Math.max(y - 1, 0)

            let next_amplitude = this.buffer_1[x][top] + this.buffer_1[x][bottom] + this.buffer_1[right][y] + this.buffer_1[left][y]
            next_amplitude >>= 1
    ...
    ripple(x, y) {
        ...
        let left = Math.max(x - this.ripple_radius, 0),
            ...
            bottom = Math.min(y + this.ripple_radius, this.buffer_1[0].length - 1)
